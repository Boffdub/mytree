...
export default function AnswerScreen({ navigation, route }) {
  ...
  const hasNextQuestion = questionIndex !== null && questionIndex + 1 < totalQuestions;
  ...
  const selectedAnswerText = question && selectedAnswer !== null ? question.options[selectedAnswer] : null;
  ...
  useEffect(() => {
    // Include quizId in the key to prevent collisions across replays
    const questionKey = question
      ? `${quizSession.quizId}-${category}-${question.id}-${questionIndex}`
      : null;
  ...
  }, [question?.id, questionIndex, category, selectedAnswer, isCorrect, quizSession.quizId,
      incrementScore, decrementScore, scoreAlreadyUpdated, saveAnswer, completeSession, hasNextQuestion]);
...
const styles = StyleSheet.create({
  ...
  correctBanner: { backgroundColor: '#1E8F2D' },
  incorrectBanner: { backgroundColor: '#F44336' },
  resultText: {
    color: '#fff',
    fontSize: 24,
    fontWeight: 'bold',
  ...
  answerDisplayContainer: { width: '100%', marginBottom: 20 },
  answerBox: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingVertical: 12,
  ...
  correctAnswerBox: { backgroundColor: '#CEE7CF', borderColor: '#1E8F2D' },
  answerLetterCircle: {
    width: 30,
    height: 30,
    borderRadius: 15,
  ...
  incorrectAnswerBox: { backgroundColor: colors.errorRedLight, borderColor: colors.errorRed },
